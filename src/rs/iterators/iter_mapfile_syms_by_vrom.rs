...
#[derive(Debug, Clone)]
pub struct IterMapFileSymsByVrom<'map> {
    mapfile: &'map MapFile,
    index: usize,
    address: u64,
}

impl<'map> IterMapFileSymsByVrom<'map> {
    ...
    fn next_impl(&mut self) -> Option<MaybeFoundSymbolInfo<'map>> {
        while self.index < self.mapfile.segments_list.len() {
            ...
            if let Some(sym) = segment.find_possible_symbol_by_vrom(self.address) {
                return Some(sym);
            }
        }
...
impl<'map> Iterator for IterMapFileSymsByVrom<'map> {
...
impl<'map> FusedIterator for IterMapFileSymsByVrom<'map> {}
